...
def scalar_vector_product(x, y):
    ...
    return (
        [scalar_vector_product(x, _y) for _y in y] if hasattr(y, "__iter__") else x * y
    )
...
def cross_entropy_loss(x, y):
    ...
    return (-1.0 / len(x)) * sum(
        x * np.log(_y) + (1 - _x) * np.log(1 - _y) for _x, _y in zip(x, y)
    )
...
def gaussian(mean, st_dev, x):
    ...
    return (
        ...
        * np.exp(-0.5 * (float(x - mean) / st_dev) ** 2)
    )
